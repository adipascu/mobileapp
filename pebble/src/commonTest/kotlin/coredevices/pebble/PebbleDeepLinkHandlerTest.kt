...
class PebbleDeepLinkHandlerTest {
    ...
    @Test
    fun indexNavbarLinksRequireIndexHardware() {
        assertEquals(null, navbarRoute("/index", indexHardwareEnabled = false))
        assertEquals(
            PebbleNavBarRoutes.IndexRoute,
            navbarRoute("/index", indexHardwareEnabled = true),
        )
        assertEquals(null, navbarRoute("/watches", indexHardwareEnabled = true))
    }
}
